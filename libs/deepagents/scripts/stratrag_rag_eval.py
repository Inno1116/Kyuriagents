...
from deepagents.runtime.dashscope import create_dashscope_embed_query

if TYPE_CHECKING:
    from collections.abc import Iterable, Mapping

    from deepagents.rag import StratRAGExample
...
def _init_elasticsearch(config: AgentRuntimeConfig, *, reset: bool) -> None:
    spec = _json_resource("deepagents.rag", "schemas/elasticsearch_index.json")
    es = _elasticsearch(config)
...
def _init_milvus(config: AgentRuntimeConfig, *, reset: bool) -> None:
    ...
    embedding = cast("dict[str, object]", _json_resource("deepagents.rag", "schemas/milvus_collection.json")["embedding"])
...
def _milvus_schema(dimension: int) -> _MilvusSchema:
    ...
    spec = _json_resource("deepagents.rag", "schemas/milvus_collection.json")
...
def _create_embed_documents(config: AgentRuntimeConfig, *, embedding_batch_size: int) -> EmbedDocuments:
    ...
    try:
        ...
        msg = "Install `deepagents[runtime]` to use DashScope embeddings."
        raise ImportError(msg) from exc
